  ...
  <view class="container">
    ...
    <view class="cover-container"
      ...
      <view class="tj-sction"
            v-if="hasLogin">
        <view class="tj-item"
              @click="navTo('/pages/user/surplusMoneny')">
          <view class="dj"><text>余额总数</text> {{ userInfo.yezong }}</view>
          <!-- <text style="color: #323F6C;">余额</text> -->
          <view class="dj"><text>可用余额</text> {{ userInfo.Points }}</view>
          <view class="dj"><text>冻结余额</text> {{ userInfo.FrozenBalance }}</view>
        </view>
        <view class="tj-item"
              @click="navTo('/pages/integralPages/assetManagement/index')">
          <view class="dj"><text>积分总额</text> {{userInfo.fczong}}</view>
          <view class="dj"><text>可用积分</text> {{userInfo.FishCoin}}</view>
          <view class="dj"><text>冻结积分</text> {{userInfo.FrozenFishCoin}}</view>
        </view>
      </view>
      <!-- 订单 -->
      <view class="order-section-hd">
        <view class="orderMy">我的订单</view>
        <view class="allOrder"
              @click="navTo('/pages/order/order?state=0')">查看全部订单 ></view>
      </view>
      <view class="order-section">
        ...
        <view class="order-item"
              @click="navTo('/pages/order/tuikuang?state=4&title=退货/售后')"
              hover-class="common-hover"
              :hover-stay-time="50">
          <image src="../../static/tuihuo.png"
                 mode=""></image>
          <text>退款/售后</text>
        </view>
      </view>
      ...
      <view class="history-section icon">
        <!-- <list-cell title="地址管理" @eventClick="navTo('/pages/address/address')"></list-cell> -->
        <list-cell v-if="false"
                   title="我的分享"
                   @eventClick="navTo('/pages/user/share')"></list-cell>
        <list-cell title="我的收藏"
                   @eventClick="navTo('/pages/collection/collection')"></list-cell>
        <list-cell title="我的粉丝"
                   @eventClick="
            navTo('/pages/user/teamManagement/createTeam/createTeam')
          "></list-cell>
        <list-cell title="联系客服"
                   @eventClick="navTo('/pages/user/service')"></list-cell>
        <list-cell title="设置"
...
export default {
  ...
  onShow () {
    this.phoneName =
      uni.getStorageSync('phone').replace(/(\d{3})\d{4}(\d{4})/, '$1****$2') ||
  ...
  methods: {
    ...
    async getUserInfo () {
      ...
      if (res.data.status == true) {
        ...
        //   (res.data.Data.Points * 10 + res.data.Data.FrozenBalance * 10) / 10;
