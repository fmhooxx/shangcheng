  ...
  <view class="container">
    ...
    <view class="introduce-section">
      ...
      <view class="price-box">
        ...
        <text v-if="shopDetails.Unit == '' ? false:true">/{{shopDetails.Unit}}</text>
    ...
    <view class="c-list">
      ...
      <view class="c-row b-b">
        ...
        <view class="bz-list con">
          <!-- <text>{{shopDetails.SevenDayTuikuan}}</text> -->
          <text>七天内可换货总金额的10%</text>
          <!-- <text>7天无理由退换货</text>
          <text>生活超市类产品除外</text>
          <text>七天内可换货(不超过订单金额的10%, 前提是不影响二次销售)</text> -->
        </view>
      </view>
      <!-- <view class="service">
        <view class="service-one">服务</view>
    ...
    <view class="page-bottom">
      ...
      <view class="action-btn-group">
        <button type="primary"
                ...
                @click="footerBuy(2)">立即购买</button>
        <button type="primary"
                class="action-btn no-border add-cart-btn"
                @click="footerJoinCart(3)">加入购物车</button>
      </view>
    </view>
...
export default {
  ...
  filters: {
    ...
    //   let result = ''
    //   if (val == 0) {
    //     return result = '七天内可换货总金额的10%'
    //   }
    //   if (val == 1) {
    //     return result = '七天无理由退换货'
    //   }
    // }
  },
  methods: {
    ...
    async getShopDetails () {
      ...
      if (res.data.status == true) {
        ...
        obj.Unit = res.data.Data.Unit
        obj.SevenDayTuikuan = res.data.Data.SevenDayTuikuan
        // 图文详情富文本内容
        let result = res.data.Data.Description.replace(/(\<img|src=")/gi, ($0, $1) => {
          return {
        ...
        if (res.data.Data.ImageUrl1 != '') {
          this.swiperList.push(this.$config.domain.api + res.data.Data.ImageUrl1);
        }
        if (res.data.Data.ImageUrl2 != '') {
          this.swiperList.push(this.$config.domain.api + res.data.Data.ImageUrl2);
        }
        if (res.data.Data.ImageUrl3 != '') {
          this.swiperList.push(this.$config.domain.api + res.data.Data.ImageUrl3);
        }
        if (res.data.Data.ImageUrl4 != '') {
          this.swiperList.push(this.$config.domain.api + res.data.Data.ImageUrl4);
        }
        if (res.data.Data.ImageUrl5 != '') {
          this.swiperList.push(this.$config.domain.api + res.data.Data.ImageUrl5);
        }
