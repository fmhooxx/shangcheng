<template>
  <view class="content">
    ...
    <view class="store">
      <view v-for="(item,index) in orderDetail.LineItemsList"
            :key='index'>
      ...
      <view class="storeFooter">
        ...
        <ul>
          ...
          <li>订单编号: {{orderDetail.OrderId}}<span class="copy"
                  @click="copyInfo(orderDetail.OrderId,'订单号复制成功')">复制</span></li>
    ...
    async getShopDetail(id) {
      ...
      if (res.data.status == true) {
        let result = res.data.Data
				this.ProductId = result.LineItemsList[0].ProductId
			// this.ProductId = result.LineItems['1865'].ProductId
